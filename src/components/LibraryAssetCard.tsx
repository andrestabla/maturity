  ...
  return (
    ...
    >
      ...
      >
        ...
        {asset.openAccess && (
          <div className="absolute top-2 right-2 bg-emerald-500 text-white text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md shadow">OA</div>
        )}
        {isInstitutional && (
          <div className="absolute top-2 right-2 bg-indigo-600 text-white text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md shadow">🔒 INST</div>
        )}
        <div className="absolute bottom-2 left-2 z-10" onClick={(e) => { e.stopPropagation(); onToggleSelect?.(asset.id, !isSelected); }}>
          <input type="checkbox" className="w-4 h-4 rounded border-white/60 text-ocean focus:ring-ocean accent-ocean cursor-pointer shadow"
            checked={isSelected} onChange={(e) => { e.stopPropagation(); onToggleSelect?.(asset.id, e.target.checked); }}
            onClick={(e) => e.stopPropagation()} aria-label="Seleccionar recurso" />
        </div>
      </div>
      <div className="library-card__body flex flex-col flex-grow p-3 bg-white border border-t-0 border-line/20 rounded-b-2xl">
        <div className="flex items-center justify-between gap-1 mb-2">
          <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded truncate max-w-[70%]"
        ...
        <div className="flex items-center gap-2 mt-1">
          {yearLabel && <span className="text-[10px] text-muted font-medium">{yearLabel}</span>}
        ...
        <div className="library-card__actions flex items-center gap-1 mt-2 pt-2 border-t border-line/10" onClick={(e) => e.stopPropagation()}>
          <button type="button" onClick={(e) => { e.stopPropagation(); onPreview(asset); }}
            className="flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-slate-100 text-muted hover:text-ink transition-colors text-[10px] font-bold" title="Vista previa">
            <Eye size={12} /><span>Preview</span>
          </button>
          {asset.canonicalUrl && (
            <a href={asset.canonicalUrl} target="_blank" rel="noopener noreferrer"
              className="p-1.5 hover:bg-slate-100 text-muted hover:text-ink rounded-lg transition-colors" title="Ver fuente"
              onClick={(e) => e.stopPropagation()}>
              <ExternalLink size={12} />
            </a>
          )}
          <button type="button" className="ml-auto p-1.5 bg-ink text-white hover:bg-ocean rounded-lg transition-all shadow-sm active:scale-90"
            onClick={(e) => { e.stopPropagation(); onAddToCourse(asset); }} title="Añadir al curso">
            <Plus size={13} />
          </button>
        </div>
      </div>
